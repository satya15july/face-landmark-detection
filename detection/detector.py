...
class Detector(object):
    ...
    #datasize:24 or 48
    def __init__(self, net_factory, data_size, batch_size, model_path, profiling = False):
        self.profiling = profiling
	    #print("Detector path : ", model_path)
        graph = tf.Graph()
        with graph.as_default():
            ...
            if self.profiling == True:
                ...
                if flops is not None:
                    print('Detector FLOPs : ', flops.total_float_ops)
    ...
    def predict(self, databatch):
        ...
        for idx, data in enumerate(minibatch):
            ...
            if self.profiling == True:
                ...
                cls_prob, bbox_pred,landmark_pred = self.sess.run([self.cls_prob, self.bbox_pred,self.landmark_pred], feed_dict={self.image_op: data}, options=options, run_metadata=run_metadata)
                fetched_timeline = timeline.Timeline(run_metadata.step_stats)
                chrome_trace = fetched_timeline.generate_chrome_trace_format()
                with open('Detector.json', 'wa') as f:
